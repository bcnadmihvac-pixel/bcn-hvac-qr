...
const EMPRESAS = [{ key: "capex", label: "capex" }];
...
function makeCapex38() {
  return [
    ...
    { id: "capex-33", empresa: "capex", nombre: "TRANE 6000 R-410", tipo: "split", estado: "OK", ultimoServicio: hoyISO(), split: {}, rooftop: null, notas: "" },{ id: "capex-34", empresa: "capex", nombre: "TRANE 2250 R410", tipo: "split", estado: "OK", ultimoServicio: hoyISO(), split: {}, rooftop: null, notas: "" },
...
export default function App() {
  ...
  const [admin, setAdmin] = useState(() => localStorage.getItem(LS_ADMIN) === "1");
  const [pinTry, setPinTry] = useState("");
  const [selected, setSelected] = useState(null);
  const [draft, setDraft] = useState(null);
  const [showQR, setShowQR] = useState(false);
  const [openSec, setOpenSec] = useState({ capex: true });
  ...
  const makeLink = (id) => window.location.origin + window.location.pathname + `?id=${id}`;

  const porEmpresa = useMemo(() => {
    const map = { capex: [] };
    for (const it of items) if (map[it.empresa] !== undefined) map[it.empresa].push(it);
    return map;
  }, [items]);
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      {selected && (
        <div className="fixed inset-0 bg-white overflow-y-auto">
          ...
          <div className="p-4 grid gap-5"><section className="grid gap-3">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <span className="block text-[11px] text-neutral-500">Empresa</span>
                  <span className="font-medium">{selected.empresa.toUpperCase()}</span>
                </div>
                <div>
                  <span className="block text-[11px] text-neutral-500">Tipo</span>
            ...
            <div className="sticky bottom-0 p-3 bg-white/90 border-t">
              <div className="flex items-center gap-2">
                ...
                {!isKiosk && (
                  <a className="h-11 px-4 rounded border grid place-items-center" href={`?id=${encodeURIComponent(selected.id)}`}>
                    Link directo
                  </a>
                )}
              </div>
  ...
  );function FieldNumber({ admin, label, value, onChange, step = 1 }) {
  return (
    <div>
